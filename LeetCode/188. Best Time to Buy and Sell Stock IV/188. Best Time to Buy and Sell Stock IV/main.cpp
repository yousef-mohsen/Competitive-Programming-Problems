...
public:
    ...
    int maxProfit(int k, vector<int>& prices) {
        ...
        dp = vector<vector<int>>(k + 1, vector<int>(size, 0));
        ...
        for(int i = 1; i <= k; i++)
        {
            ...
            {
                int mn = prices[j], mx = 0;
                
                for(int l = j + 1; l < size; l ++)
                {
                    
                    mx = max(prices[l] - mn, mx);
                    mn = min (mn, prices[l]);
                    if(j == 0)
                    {
                        dp[i][l] = max(mx, dp[i - 1][l]);
                    }
                    else
                    {
                        dp[i][l] = max( dp[i][l],max(mx + dp[i - 1][j - 1], dp[i - 1][l]));
                    }
